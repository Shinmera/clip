...
(define-tag-processor case (node)
  ...
  (let* ((parent (plump:parent node))
    ...
    (or (loop for child across (plump:children node)
              when (plump:element-p child)
              do (when (loop for key being the hash-keys of (plump:attributes child) using (hash-value val)
                             thereis (and (string-equal "value" key) (equal value (parse-and-resolve val))))
